...
![](../assets/gofs-criacionais/factory_method.jpeg)
<center>
  <figure>
...
Segue abaixo uma possivel implementação do factory method nesse contexto utilizando o Kotlin.

    interface Chat {
        val mensagem: String
        fun criar(mensagem: String)
        fun ler()
    }
    
    class ChatUsuario : Chat {
        override var mensagem: String = ""

        override fun criar(mensagem: String) {
            this.mensagem = mensagem
            println("Chat criado para usuário: $mensagem")
        }

        override fun ler() {
            println("Lendo mensagem do usuário: $mensagem")
        }
    }
    
    class ChatAdministracao : Chat {
        override var mensagem: String = ""

        override fun criar(mensagem: String) {
            this.mensagem = mensagem
            println("Chat criado para administração: $mensagem")
        }

        override fun ler() {
            println("Lendo mensagem da administração: $mensagem")
        }
    }
    ...
    class UsuarioComum : Usuario() {
        ...
        var interesses: List<String> = listOf()

        override fun criarChat(): Chat {
            return ChatUsuario()
        }
    }
...
Durante o desenvolvimento do Módulo Padrões de Projeto GoF(s) Criacionais, nossa equipe se organizou de forma eficiente para modelar e implementar os padrões Factory Method e Builder. A equipe foi dividida conforme consta no tópico 2 de Metodologia apresentado nesse artefato. 

A equipe teve discussões importantes sobre a direção estratégica para escolha dos padrões a serem utilizados, levantando questionamentos principalmente no que tange quais critério utilizar para decidir quando criar uma nova subclasse de fábrica ou como garantir que a fábrica possa ser estendida de forma a acomodar novos tipos de produtos sem modificar as classes existentes. 

Tais discussões, além das revisões e alterações apontadas pelos Revisores Principais e Revisores Secundários, ajudaram a equipe a alinhar as expectativas e garantir que a implementação dos padrões de projeto fosse eficaz e alinhada com os objetivos do projeto.
...
|1.5|   24/07/2024  |  Adiciona texto e modelagem sobre Builder. |  [Ana Letícia](https://github.com/analeticiaa) | [Augusto Duarte](https://github.com/Augcamp) |  24/07/2024 |
|1.6|   24/07/2024  |  Adiciona codigo do builder e referencia. |  [Cleber Brant](https://github.com/Cleberbrant) | [Augusto Duarte](https://github.com/Augcamp) |  24/07/2024 |
...
|1.8|   25/07/2024  |  Adicionando seção "colaboração e trabalho em equipe"  |  [Suzane Duarte](https://github.com/suzaneduarte) | [Edilberto Cantuaria](https://github.com/edilbertocantuaria)  |   25/07/2024| 